...
import { sentenceTransformerPluginNode } from "./nodes/SentenceTransformerPluginNode.js";
...
const plugin: RivetPluginInitializer = (rivet) => {
  ...
  const sentenceTransformerNode = sentenceTransformerPluginNode(rivet);

  // The plugin object is the definition for your plugin.
  const sentenceTransformerPlugin: RivetPlugin = {
    // The ID of your plugin should be unique across all plugins.
    id: "sentence-transformer-plugin",

    // The name of the plugin is what is displayed in the Rivet UI.
    name: "Sentence Transformer Plugin",
    ...
    register: (register) => {
      register(sentenceTransformerNode);
    },
  };

  // Make sure to return your plugin definition.
  return sentenceTransformerPlugin;
};
